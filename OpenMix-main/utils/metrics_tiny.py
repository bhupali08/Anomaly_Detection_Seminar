...
def calc_metrics(args, loader, model, criterion):
    acc, softmax, correct, logit, conf_correct, conf_wrong, ece, nll = get_metric_values(args, loader, model, criterion)
    # aurc, eaurc
    aurc, eaurc = calc_aurc_eaurc(softmax, correct)
    # fpr, aupr
    auroc, aupr_success, aupr, fpr, tnr = calc_fpr_aupr(softmax, correct)

    return acc, auroc*100, aupr_success*100, aupr*100, fpr*100, tnr*100, aurc*1000, eaurc*1000, ece*100, nll*10
...
def get_metric_values(args, loader, model, criterion):
    ...
    return total_acc, list_softmax, list_correct, list_logit, conf_correct, conf_wrong, ece, nll
