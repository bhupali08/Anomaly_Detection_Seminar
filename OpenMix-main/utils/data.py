...
class RandomImages(torch.utils.data.Dataset):
    def __init__(self, transform=None, exclude_cifar=True, data_num=200):
...
def get_loader(data, data_path, batch_size, args):
    ...
    if args.method == 'CS-KD':
        ...
        train_loader = DataLoader(trainset, batch_sampler=get_train_sampler(trainset), num_workers=0)
        valid_loader = DataLoader(evalset, batch_sampler=get_test_sampler(evalset), num_workers=0)
        test_loader = DataLoader(testset, batch_sampler=get_test_sampler(testset), num_workers=0)
    else:
        method = None
        ...
        valid_loader = DataLoader(eval_data, batch_size=batch_size, shuffle=False, num_workers=0)
