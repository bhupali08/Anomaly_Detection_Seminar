...
def classAug(args, x, y, base_numclass, alpha=10.0, mix_times=2):
    ...
    y = torch.cat((y, new_target.long()), 0)
...
def mixup_data(x, y, alpha=0.3, use_cuda=False):
    lam = np.random.beta(alpha, alpha)
    batch_size = x.size()[0]
    if use_cuda:
...
def RegMixup_data(x, y, alpha=10, use_cuda=False):
    lam = np.random.beta(alpha, alpha)
    batch_size = x.size()[0]
    if use_cuda:
        index = torch.randperm(batch_size).cuda()
...
def train(loader, loader_out, model, criterion, criterion_ranking, optimizer, epoch, history, logger, args):
    ...
    if args.method == 'OpenMix':
        for in_set, out_set in zip(loader, loader_out):
            ...
            in_data, out_data, target = in_set[0], out_set[0], in_set[1]
            in_oe, lam = OE_mixup(in_data, out_data)
            inputs = torch.cat([in_data, in_oe], dim=0)
            logits = model(inputs)
            target_oe = torch.LongTensor(in_oe.shape[0]).random_(args.classnumber, args.classnumber + 1)
            target=target.long()
        ...
        for in_set, out_set in zip(loader, loader_out):
            ...
            target_oe = torch.LongTensor(logits_oe.shape[0]).random_(args.classnumber, args.classnumber + 1)
            target=target.long()
