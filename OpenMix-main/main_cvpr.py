...
# Then conditionally use the resource module
if platform.system() != "Windows":
...
def main():
    ...
    save_path = f"{args.save_path}{args.data}_{args.model}_{args.method}"
    ...
    for r in range(args.run):
        ...
        if args.model == 'resnet18':
            ...
            model = densenet_BC.DenseNet3(depth=100, num_classes=num_class,
                                          ...
                                          bottleneck=True, dropRate=0.0).to(device)
        elif args.model == 'vgg':
            model = vgg.vgg16(**model_dict).to(device)
        elif args.model == 'wrn':
            model = wrn.WideResNet(28, num_class, 10).to(device)
        elif args.model == 'efficientnet':
            model = efficientnet.efficientnet(**model_dict).to(device)
        elif args.model == 'mobilenet':
            model = mobilenet.mobilenet(**model_dict).to(device)
        elif args.model == "cmixer":
            model = convmixer.ConvMixer(256, 16, kernel_size=8, patch_size=1, n_classes=num_class).to(device)
            
   # Loss, Optimizer, and Scheduler Configuration
...
# Logger and History Initialization
...
# start Train(Outlier Data Loading and Training Loop)
        ...
        train_loader_out = torch.utils.data.DataLoader(ood_data,
            batch_size=args.aux_batch_size, shuffle=True, num_workers=0, pin_memory=True)
